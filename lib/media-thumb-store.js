 ...
 * @constructor
 */
var mediaThumbStore = function(options) {
  // Default settings
  var _this = this;
  var settings = {
    mimeList: ['image/jpeg',
  ...
  /**
   * Get one data object by file path
   *
   * @param {String} path
   * @param {requestCallback} callback
   *
   * @return mediaThumbStore
   */
  this.findByPath = function(path, callback) {
    var query = {
      fields: ['path', 'name', 'mime', 'tags'],
      where: ['path==' + path]
    };
  ...
  /**
   * Insert new data object to data store
   * Does not check if path exist in data store
   *
   ...
   * @return mediaThumbStore
   */
  this.insert = function(mediaPath, callback) {
    // Get media mime
   ...
   * Only insert new path if not already in data store
   *
   * @param {String} mediaPath
   * @param {requestCallback} callback
   *
  ...
  this.create = function(mediaPath, callback) {
    // Look for the path in the data store
    this.findByPath(mediaPath, function(err, results) {
      if (results.length) {
        callback(new Error('Path already exist'));
      } else {
        _this.insert(mediaPath, callback);
      }
    });
  };
